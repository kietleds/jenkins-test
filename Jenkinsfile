    ...
    post {
        ...
        failure {
            script {
                ...
                def response = sh(script: """
                    curl -H "Content-Type:application/json" -H "Authorization:JWT $token" --data '{ "buildURL": "'"$env.BUILD_URL"'", "tool":"jenkins", "result":"failed"  }' "https://kiet-le-dss-macbook-air.tail305ff.ts.net/ds/test-executions/${TEST_EXECUTION_KEY}/pipeline/history?projectKey=${PROJECT_KEY}"
                """, returnStdout: true).trim()
                echo "API Response: ${response}"
            }
        }
